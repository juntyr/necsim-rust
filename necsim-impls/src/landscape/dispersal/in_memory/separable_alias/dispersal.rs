...
use crate::landscape::dispersal::{Dispersal, SeparableDispersal};

use super::InMemorySeparableAliasDispersal;

impl Dispersal for InMemorySeparableAliasDispersal {
    #[must_use]
    ...
    fn sample_dispersal_from_location(&self, location: &Location, rng: &mut impl Rng) -> Location {
        let self_dispersal_at_location = self.get_self_dispersal_probability(location);
...
#[contract_trait]
impl SeparableDispersal for InMemorySeparableAliasDispersal {
    #[must_use]
    #[debug_requires(self.habitat_extent.contains(location), "location is inside habitat extent")]
    #[debug_ensures(self.habitat_extent.contains(&ret), "target is inside habitat extent")]
