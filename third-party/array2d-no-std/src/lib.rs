...
//! image processing, game boards, and other situations. `Array2D` cannot be
//! used when rows or columns might have different lengths⁠—all rows and columns
...
//! [`get_row_major`]:
//! struct.Array2D.html#method.get_row_major
//! [`get_mut_row_major`]:
//! struct.Array2D.html#method.get_mut_row_major
//! [`set_row_major`]:
//! struct.Array2D.html#method.set_row_major
//! [`get_column_major`]:
//! struct.Array2D.html#method.get_column_major
//! [`get_mut_column_major`]:
//! struct.Array2D.html#method.get_mut_column_major
//! [`set_column_major`]:
//! struct.Array2D.html#method.set_column_major
//! [`Vec`]:
...
impl<T> Array2D<T> {
        ...
        T: Clone,
    {
        ...
        Ok(Array2D {
            array: flatten(elements).into_boxed_slice(),
    ...
    where
        T: Clone,
    {
        let column_len = elements.get(0).map_or(0, Vec::len);
...
fn flatten<T: Clone>(nested: &[Vec<T>]) -> Vec<T> {
    nested.iter().flat_map(Vec::clone).collect()
}
