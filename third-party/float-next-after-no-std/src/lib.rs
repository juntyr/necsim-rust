...
/// let next = big_num.next_after(f64::INFINITY);
/// assert_eq!(next, 16237485966.000006_f64);
///
/// // Expected handling of 1.0
/// let one = 1_f64;
/// let next = one.next_after(f64::INFINITY);
/// assert_eq!(next, 1_f64 + f64::EPSILON);
///
/// // Tiny (negative) numbers
/// let zero = 0_f32;
/// let next = zero.next_after(f32::NEG_INFINITY);
...
mod tests_f64 {
    ...
    const POS_INF: T = f64::INFINITY;
    const NEG_INF: T = f64::NEG_INFINITY;
    ...
    const LARGEST_NEG: T = f64::MIN;

    const POS_ONE: T = 1.0;
    const NEG_ONE: T = -1.0;
    const NEXT_LARGER_THAN_ONE: T = 1.0 + f64::EPSILON;
...
mod tests_f32 {
    ...
    const POS_INF: T = f32::INFINITY;
    const NEG_INF: T = f32::NEG_INFINITY;
    const POS_ZERO: T = 0.0;
    const NEG_ZERO: T = -0.0;
    ...
    const LARGEST_NEG: T = f32::MIN;

    const POS_ONE: T = 1.0;
    const NEG_ONE: T = -1.0;
    const NEXT_LARGER_THAN_ONE: T = 1.0 + f32::EPSILON;
